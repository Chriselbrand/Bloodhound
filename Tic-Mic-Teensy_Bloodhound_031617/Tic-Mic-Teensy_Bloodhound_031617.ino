...
const int KNOCKER_SOLENOID_PULSE_PIN = 20; //pulse to MOSFET to trigger knocking solenoid
...
elapsedMillis knockLength; //this resettable elapsed delay allows the knock pulse to be stopped without delay()
...
//  if(SHOW_ME_THE_KNOCKERS == HIGH){
//     sampleSource = 14;
//     digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, HIGH);
//     knockLength = 0; //reset the elapsed timer for the knock. see samplingCallback() for when the knock is actually turned off.
//     delay(1);
//     samplingBegin();
////     if(digitalRead(KNOCKER_SOLENOID_PULSE_PIN) == HIGH){  //make sure that pin actually went low for the solenoid
...
//    Serial.println(knockLength);
    if(knockLength > 20){ //length of readings is like 255
      digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, LOW);
    }
    if(knockLength > 110){ //length of readings is like 255
      digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, HIGH);
    }
    if(knockLength > 130){ //length of readings is like 255
      digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, LOW);
    }
    if(knockLength > 235){ //length of readings is like 255
      digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, HIGH);
    }
    if(knockLength > 250){ //length of readings is like 255
      digitalWrite(KNOCKER_SOLENOID_PULSE_PIN, LOW);
      knockLength = 0;
    }
  }
...
void hollowOrSolid() {
  int averageSound = magnitudes[18];
  averageSound = averageSound / magnitudes[19];

  if(averageSound > 1500){
